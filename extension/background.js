...
let lastError = "Menunggu koneksi ke aplikasi native.";
...
function connectHost() {
  ...
  port.onDisconnect.addListener(() => {
    ...
    nativePort = null;
    connected = false;
    lastError = error || "Koneksi terputus. Mencoba menyambung ulang otomatis.";
    failures = Math.min(failures + 1, 4);
    retryAfter = Date.now() + Math.min(15000, 1000 * 2 ** failures);
    chrome.action.setBadgeText({ text: "!" });
    chrome.action.setTitle({ title: "Bridge terputus. Buka Notch Box dan periksa pemasangan native host." });
  ...
  port.onMessage.addListener(async command => {
    if (command?.protocolVersion === 1 && command.kind === "hello") {
      ...
      lastError = "";
      chrome.action.setBadgeText({ text: "" });
      chrome.action.setTitle({ title: "Notch Box: bridge lokal terhubung" });
      if (pendingSetup) {
        pendingSetup = false;
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  if (!sender.tab && sender.id === chrome.runtime.id && sender.url === chrome.runtime.getURL("popup.html")) {
    if (message?.kind === "status") {
      connectHost();
      respond({ connected, error: lastError, sources: [...sources.values()].filter(source => Date.now() - source.updatedAt < 5000).length });
