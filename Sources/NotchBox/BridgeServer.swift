...
final class BridgeServer {
    ...
    func start() throws {
        ...
        guard lockFile >= 0, flock(lockFile, LOCK_EX | LOCK_NB) == 0 else {
            ...
            throw BridgeError.system("Aplikasi Notch Box lain sudah berjalan.")
        }
        unlink(path)
        var address = try LocalSocket.address(for: path)
        listener = socket(AF_UNIX, SOCK_STREAM, 0)
        guard listener >= 0 else { throw BridgeError.system("Socket server gagal dibuat.") }
        ...
        guard result == 0, listen(listener, 2) == 0 else {
            ...
            listener = -1
            throw BridgeError.system("Bridge lokal tidak dapat dimulai.")
        }
        socketPath = path
        chmod(path, 0o600)
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            while true {
                ...
                self.send(Data(#"{"protocolVersion":1,"kind":"hello"}"#.utf8))
