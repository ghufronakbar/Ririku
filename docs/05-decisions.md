...
## Implementasi tahap B v0.3.0 — 17 September 2026

Pengguna meminta melanjutkan setelah menyetujui rencana install tanpa Terminal. Pilihan teknis agen: ID extension tetap lewat field `key`, pendaftaran native host dan penyalinan extension dilakukan app hanya lewat tombol Setup, pendaftaran diblokir saat App Translocation, dan versi extension dilaporkan ke app untuk mendeteksi extension usang. Tombol **Salin alamat** dipilih alih-alih membuka `chrome://extensions` otomatis karena perilaku membuka URL internal Chrome dari app belum divalidasi. Skrip `install-host.py` dipertahankan untuk development. Dokumen terdampak: README, 02, 03, 04, 06.
